...
stolp -> function(points, classes,errors) 
{
  ...
  plot(1:n, sort(margins), col="black", bg="blue",pch=20,
       main = "Отступы PW(Гауссовское ядро)", ylab="Отступ", xlab="Выборка")
  lines(1:n, sort(margins), lwd = 2, col = "blue")
  ...
  while(n!=length(etalone))
  {
    ...
    if( count < errors )
    {
      plot(pointsWE[,1:2],col = colors[classes], pch = 21, asp = 1,main = "STOLP PW(Гауссовское ядро)")
